...
import { Lightbulb, Plus, X, Trash2, Clock, Edit3, Search, Sparkles, Loader2 } from 'lucide-react';
...
const IdeasView: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    await addNote({ title, content, projectId: 'GLOBAL', createdAt: new Date() });
    resetForm();
  };
  ...
  const handleBrainstorm = async () => {
    ...
    const newIdeas = await GeminiService.generateIdeas(existingTitles);

    for (const idea of newIdeas) {
      await addNote({
        title: idea,
        content: "Generated by FP-Engine based on your existing knowledge base.",
        projectId: 'GLOBAL',
        createdAt: new Date()
      });
    }
    ...
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {isBrainstorming ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
      ...
      {filteredNotes.length === 0 ? (
        <div className="glass-card-subtle p-20 text-center rounded-3xl border-2 border-dashed border-white/10">
          ...
          <p className="text-sm text-white/30 mb-8 max-w-md mx-auto">
            {searchQuery ? 'Try a different search term' : 'Start capturing your ideas and thoughts. Or let AI help.'}
          </p>
          {!searchQuery && (
            <div className="flex justify-center gap-4">
              ...
              <GlassButton onClick={handleBrainstorm} disabled={isBrainstorming}>
                <Sparkles size={18} className="mr-2" />
                {isBrainstorming ? "Generating..." : "Auto-Generate Ideas"}
              </GlassButton>
            </div>
          )}
        </div>
